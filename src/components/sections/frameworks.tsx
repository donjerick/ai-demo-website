...
const tools = zipAIToolkit.getTools();`
  },
  {
    ...
    description: "Use the Model Context Protocol to add payment capabilities to any MCP-compatible AI system.",
      ...
      <div className="max-w-7xl mx-auto px-0 md:px-10">
        <div className="text-center mb-16 px-6 md:px-0">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Our toolkit integrates seamlessly with all major AI frameworks, allowing you to add payment capabilities to your existing AI applications.
          </p>
        </div>
        ...
        >
          <Tabs defaultValue="openai" className="w-full">
            <TabsList className="grid grid-cols-2 md:grid-cols-4 mb-8 w-full bg-background/50 backdrop-blur-sm border border-border/30 p-1.5 rounded-xl px-6 md:px-0">
            ...
            {frameworks.map((framework) => (
              <TabsContent key={framework.id} value={framework.id}>
                <Card className="border border-border/50 bg-card/50 backdrop-blur-sm rounded-none md:rounded-xl overflow-hidden shadow-md mt-12">
                  <CardHeader>
                  ...
                  <CardContent>
                    <div className="relative">
                      ...
                      <div className="flex items-center justify-between bg-secondary/40 px-4 py-2 rounded-t-lg border-b border-border/30">
                        <div className="flex items-center space-x-2">
                          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 32 32" fill="#539E43">
                      ...
                      <div className="h-[300px] overflow-y-auto scrollbar-custom bg-secondary/20 rounded-b-lg">
                        <Highlight
                        ...
                        >
                          {({ className, style, tokens, getLineProps, getTokenProps }) => (
                            <pre className={`${className} p-4 m-0 text-sm font-mono`} style={{ ...style, background: 'transparent' }}>
                              {tokens.map((line, i) => (
                                <div key={i} {...getLineProps({ line })} className="flex">
                                  <span className="text-right mr-4 select-none opacity-40 text-xs min-w-[2rem] inline-flex justify-end" style={{ userSelect: 'none' }}>
                                    {i + 1}
                                  </span>
                                  <span>
                                    {line.map((token, key) => (
